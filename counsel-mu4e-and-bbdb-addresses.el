...
(defvar counsel-mu4e-annd-bbdb-addresses-mu4e-contacts-history nil
  "Stores history of contact completions with counsel-mu4e-annd-bbdb-addresses-mu4e-contacts.")

;; HT to store contacts
(defvar counsel-mu4e-annd-bbdb-addresses-mu4e-contacts-extended nil
  "HT that stores mu4e~contexts and bbdb contacts for mu4e completion.")

;; HT to replace mu4e~contacts
(defvar counsel-mu4e-annd-bbdb-addresses-mu4e-contacts nil
  "Replaces `mu4e~contacts'.")

;; contacts-bbdb replacement that also has mu4e contacts
(defvar counsel-mu4e-annd-bbdb-addresses-counsel-bbdb-contacts nil
...
;; add bbdb contacts to mu4e
				          ...
				          (defun counsel-mu4e-annd-bbdb-addresses-mu4e-extract-email-from-address (add)
				          ...
				          (defun counsel-mu4e-annd-bbdb-addresses-mu4e-extract-name-from-contact (add)
				          ...
				          (defun counsel-mu4e-annd-bbdb-addresses-mu4e-extract-name-or-emtpy-string (add)
				            "Extract name from a mu4e-contact if it of the form 'name <email>', return empty string if this is only an email address."
				            (let ((name (counsel-mu4e-annd-bbdb-addresses-mu4e-extract-name-from-contact add)))
				              (if name
					              name
				                ""
				                )
				          ...
				          (defun counsel-mu4e-annd-bbdb-addresses-mu4e-get-all-email-addresses-for-person (add)
				            ...
				            (let ((person (counsel-mu4e-annd-bbdb-addresses-mu4e-extract-name-from-contact add))
					              (emails (list (counsel-mu4e-annd-bbdb-addresses-mu4e-extract-email-from-address add)))
					              )
				              (mu4e~request-contacts-maybe)
				              (when person
				                (setq emails (append emails (cl-remove-duplicates (mapcar (lambda (a) (counsel-mu4e-annd-bbdb-addresses-mu4e-extract-email-from-address a)) (all-completions person mu4e~contacts))))
					                  )
				                )
				              emails
				              )
				          ...
				          (defun counsel-mu4e-annd-bbdb-addresses-mu4e-get-all-emails-from-person (add)
				            ...
				            (concat "(maildir:\"/lordpretzel-gmail/[lordpretzel].All Mail\" OR maildir:\"/bglavic-iit/[bglavic].All Mail\" OR maildir:/bglavic-iit/INBOX OR maildir:/lordpretzel-gmail/INBOX) AND ("
					                (mapconcat (lambda (x) (concat "from:" x)) (counsel-mu4e-annd-bbdb-addresses-mu4e-get-all-email-addresses-for-person add) " OR ") ")")
				            )

				          (defun counsel-mu4e-annd-bbdb-addresses-mu4e-extract-is-org-from-address (add)
...
(defun counsel-mu4e-annd-bbdb-addresses-create-contacts-list-from-mu4e-and-bbdb ()
  ...
  (when (eq counsel-mu4e-annd-bbdb-addresses-mu4e-contacts-extended nil)
	(setq counsel-mu4e-annd-bbdb-addresses-mu4e-contacts-extended (ht-create 'equal))
	)
  (when (eq counsel-mu4e-annd-bbdb-addresses-mu4e-contacts nil)
	(setq counsel-mu4e-annd-bbdb-addresses-mu4e-contacts (ht-create 'equal))
	)
  ...
  (let ((pos 1)
	...
	(mapc (lambda (c)
			(let* ((email (nth 4 c))
			  ...
			  (incf pos)
			  (puthash email contact counsel-mu4e-annd-bbdb-addresses-mu4e-contacts-extended)))
		  (sort
	...
	(mapc (lambda (c)
			(let* ((email (counsel-mu4e-annd-bbdb-addresses-mu4e-extract-email-from-address c))
				   (name (counsel-mu4e-annd-bbdb-addresses-mu4e-extract-name-from-contact c))
				   (contact `(:full-contact ,c :name ,name :contact-from "mu4e" :pos ,pos)))
			  (unless (ht-contains-p counsel-mu4e-annd-bbdb-addresses-mu4e-contacts-extended email)
				(incf pos)
				(puthash email contact counsel-mu4e-annd-bbdb-addresses-mu4e-contacts-extended))))
		  (ht-keys mu4e~contacts))
	;; put in hashtable for mu4e
	(mapc (lambda (c)
			(puthash (plist-get c :full-contact) (plist-get c :pos) counsel-mu4e-annd-bbdb-addresses-mu4e-contacts)
			)
		  (ht-values counsel-mu4e-annd-bbdb-addresses-mu4e-contacts-extended)
		  )
	;; create list for counsel-bbdb
	(setq counsel-mu4e-annd-bbdb-addresses-counsel-bbdb-contacts
		  (cl-union
		   ...
		   (mapcar (lambda (c)
				   ...
				   (--filter (string= (plist-get (cadr it) :contact-from) "mu4e")
							 (ht-map (lambda (k v) (list k v)) counsel-mu4e-annd-bbdb-addresses-mu4e-contacts-extended)
							 )
				   )
		   )
		  )
...
(defun counsel-mu4e-annd-bbdb-addresses-get-sorted-contacts ()
  "Return contacts sorted based on the order recoded in `counsel-mu4e-annd-bbdb-addresses-mu4e-contacts-extended'."
  (counsel-mu4e-annd-bbdb-addresses-create-contacts-list-from-mu4e-and-bbdb)
  (mapcar 'car (sort (ht-map (lambda (e c)
							 ...
							 counsel-mu4e-annd-bbdb-addresses-mu4e-contacts-extended)
					 (lambda (l r) (<= (cdr l) (cdr r)))
					 ))
  )

(defun counsel-mu4e-annd-bbdb-addresses-mu4e~compose-complete-handler (str pred action)
  "Complete address STR with predication PRED for ACTION."
  (unless counsel-mu4e-annd-bbdb-addresses-mu4e-contacts
	(counsel-mu4e-annd-bbdb-addresses-create-contacts-list-from-mu4e-and-bbdb)
	)
  (cond
   ((eq action nil)
	(try-completion str counsel-mu4e-annd-bbdb-addresses-mu4e-contacts pred))
   ((eq action t)
	(all-completions str counsel-mu4e-annd-bbdb-addresses-mu4e-contacts pred))
   ((eq action 'metadata)
...
(defun counsel-mu4e-annd-bbdb-addresses-mu4e-contacts ()
  (interactive)
  (ivy-read "Contact:"
			(counsel-mu4e-annd-bbdb-addresses-get-sorted-contacts)
			:history 'counsel-mu4e-annd-bbdb-addresses-mu4e-contacts-history
			:action '(1
					  ("a" (lambda (add)
							 (mu4e-headers-search (counsel-mu4e-annd-bbdb-addresses-mu4e-get-all-emails-from-person add))
							 )
					  ...
					  ("I" (lambda (add)
							 (insert (counsel-mu4e-annd-bbdb-addresses-mu4e-extract-email-from-address add))
							 ) "insert email address only")
					  ("W" (lambda (add)
							 (kill-new (counsel-mu4e-annd-bbdb-addresses-mu4e-extract-email-from-address add))
							 ) "copy to email address only to killring")
					  ("l" (lambda (add)
							 (let* ((email (counsel-mu4e-annd-bbdb-addresses-mu4e-extract-email-from-address add))
					  ...
					  ("b" (lambda (add)
							 (let* ((email (counsel-mu4e-annd-bbdb-addresses-mu4e-extract-email-from-address add)))
							   ...
							   (counsel-mu4e-annd-bbdb-addresses-counsel-bbdb-open-in-bbdb-action `(nil nil nil nil ,email))
							   )
							 )
					   "open record in BBDB"
					   )
					  )
			:caller 'counsel-mu4e-annd-bbdb-addresses-mu4e-contacts
			)
  )
...
(defun counsel-mu4e-and-bbdb-addresses-setup ()
  ...
  (advice-tools/advice-add-if-def
   ...
   :override
   'counsel-mu4e-annd-bbdb-addresses-mu4e~compose-complete-handler)
  )
